...
if (!isset($_SESSION["role"]) || $_SESSION["role"] !== "peso_staff" || !isset($_SESSION["staff_id"])) {
...
function get_count(mysqli $conn, string $table, int $staff_id): int {
  ...
  if ($hasCreatedBy && $staff_id > 0) { $sql .= " WHERE created_by = $staff_id"; }
  $res = $conn->query($sql);
...
function get_count_by_status(mysqli $conn, string $table, array $possibleStatusColumns, array $statusValues, int $staff_id): int {
  if (!table_exists($conn, $table)) return 0;
  $columns = get_columns($conn, $table);
  ...
  if ($hasCreatedBy && $staff_id > 0) { $sql .= " AND created_by = $staff_id"; }
  $res = $conn->query($sql);
  if ($res && ($row = $res->fetch_assoc())) { return (int)$row["total"]; }
  return 0;
}
...
/* =========================
   COUNTS
========================= */
$totalPrograms = get_count($conn, "programs", 0);
$totalBeneficiaryRecords = get_count($conn, "beneficiaries", 0);
$totalBeneficiaries = get_unique_beneficiary_count($conn);
$activePrograms = get_count_by_status($conn, "programs", ["status", "program_status"], ["Active", "Ongoing", "Open", "Running"], 0);
$pendingReviews = get_count_by_status($conn, "beneficiaries", ["status", "approval_status", "review_status"], ["Pending", "For Review", "Review"], 0);

$dashboardRefreshedAt = date("M j, Y, g:i A");

/* Real beneficiary registrations for the latest six calendar months. */
$chartLabels = [];
$chartValues = [];
...
/* =========================
   RECENT PROGRAMS (SMART STATUS LOGIC APPLIED)
========================= */
$recentPrograms = [];
...
if (table_exists($conn, "programs")) {
  ...
  $hasCreatedBy = in_array("created_by", $programCols, true);
  ...
  if ($nameCol) {
    ...
    $sql = "SELECT " . implode(", ", $select) . " FROM `programs` $whereClause ORDER BY $orderBy LIMIT 5";
    $res = $conn->query($sql);
...
if (table_exists($conn, "activity_logs")) {
  $sql = "SELECT action_type AS action_title, 
          ...
          FROM activity_logs 
          WHERE staff_id = ? AND action_type NOT LIKE '%Log%' AND action_type NOT LIKE '%Auth%'
...
<head>
  ...
  <title>BENEPESO | PESO Staff Dashboard</title>
...
<link rel="stylesheet" href="system_mobile.css?v=3">
...
<div class="page-wrap">
  ...
  <aside class="side-area" id="sideArea">
    ...
    <nav class="nav-area">
      <a href="peso_staff_dashboard.php" class="nav-item active"><i class="ph ph-squares-four"></i> Dashboard</a>
      <a href="peso_staff_program.php" class="nav-item"><i class="ph ph-briefcase"></i> Program</a>
  ...
  <main class="main-area">
    <header class="top-area animate-fade-in">
      <div class="top-left">
        ...
        <div class="top-title">
          ...
          <div class="top-big">PESO Staff Dashboard</div>
          <div class="top-sub">Monitor programs, beneficiaries, and recent activities in one place.</div>
        </div>
      </div>

      <div class="top-actions">
        <a href="peso_staff_program.php" class="btn-soft"><i class="ph ph-plus-circle" style="font-size: 1.1rem;"></i> Programs</a>
        <div class="top-chip">
    ...
    <section class="dashboard-command-bar animate-fade-in" aria-label="Dashboard actions">
      ...
      <nav class="dashboard-quick-actions" aria-label="Quick actions">
        <a href="peso_staff_program.php"><i class="ph ph-clipboard-text" aria-hidden="true"></i><span>Review by batch</span></a>
    ...
    <section class="stats-grid">
      <a href="peso_staff_program.php" class="stat-card stat-card-link animate-fade-in" style="animation-delay: 0.1s;">
        <div class="stat-top">
        ...
        <div class="stat-trend trend-up"><i class="ph-bold ph-briefcase"></i> All schedules</div>
        <div class="stat-note">Published and recorded program batches.</div>
      </a>

      <a href="peso_staff_program.php" class="stat-card stat-card-link animate-fade-in" style="animation-delay: 0.2s;">
        <div class="stat-top">
          <div class="stat-label">Active Batches</div>
          <div class="stat-icon"><i class="ph-fill ph-check-circle"></i></div>
        </div>
        <div class="stat-value"><?php echo (int)$activePrograms; ?></div>
        <div class="stat-trend trend-neutral"><i class="ph-bold ph-check"></i> Approved</div>
        <div class="stat-note">Approved batches currently in progress.</div>
      </a>

      <a href="peso_staff_beneficiaries.php" class="stat-card stat-card-link animate-fade-in" style="animation-delay: 0.3s;">
        <div class="stat-top">
      ...
      <a href="peso_staff_program.php" class="stat-card stat-card-link animate-fade-in" style="animation-delay: 0.4s;">
        <div class="stat-top">
        ...
        <div class="stat-note">Applications waiting for staff review.</div>
      </a>
    </section>
    ...
    <section class="chart-section animate-fade-in" style="animation-delay: 0.5s;" aria-label="Dashboard analytics">
      ...
      <div class="panel-card dashboard-chart-card dashboard-chart-card--status">
        <div class="panel-head">
          ...
          <a href="peso_staff_program.php" class="panel-link">Review by batch</a>
        </div>
    ...
    <section class="split-grid">
      <div class="panel-card animate-fade-in" style="animation-delay: 0.6s; padding-bottom: 20px;">
        <div class="panel-head">
          ...
          <a href="peso_staff_program.php" class="panel-link">View all</a>
        </div>
              ...
              <?php else: ?>
                ...
                ?>
                  <tr>
                    ...
                    <td class="col-action">
                        <a href="peso_staff_program.php?program=<?php echo urlencode($program["name"]); ?>" class="btn-action-view">
      ...
      <div class="panel-card animate-fade-in" style="animation-delay: 0.7s;">
        <div class="panel-head">
          <div>
            ...
            <div class="panel-sub">Recent meaningful system events</div>
          </div>
          <a href="peso_staff_activity_log.php" class="panel-link">Open log</a>
        </div>
