      ...
      if (!$selectedIds || ($needsBulkMessage && $bulkMessage === '') || ($needsBulkDate && $bulkDate === '') || ($needsBulkPlace && $bulkPlace === '')) {
          ...
          foreach ($selectedIds as $selectedId) {
              ...
              if ($bulkStatus === 'Ongoing' && ($transition['current'] ?? '') === 'Orientation'
                  && !tupad_confirm_documents_for_orientation($conn, $selectedId, 'peso_staff', $peso_staff_id)) {
                  $blocked++;
                  continue;
              }
  ...
  if ($action === "quick_update_status") {
      ...
      if ($beneficiary_id > 0) {
          ...
          if ($availment_status === 'Ongoing' && ($transition['current'] ?? '') === 'Orientation'
              && !tupad_confirm_documents_for_orientation($conn, $beneficiary_id, 'peso_staff', $peso_staff_id)) {
              $_SESSION["show_error_modal"] = true;
  ...
  if ($action === "admin_add_beneficiary" || $action === "admin_update_beneficiary") {
      ...
      if ($action === "admin_update_beneficiary" && $bid > 0) {
          ...
          if ($previous_availment_status !== null && $previous_availment_status !== $availment_status) {
              ...
              if ($availment_status === 'Ongoing' && ($transition['current'] ?? '') === 'Orientation'
                  && !tupad_confirm_documents_for_orientation($conn, $bid, 'peso_staff', $peso_staff_id)) {
                  $_SESSION['show_error_modal'] = true;
                  $_SESSION['error_modal_message'] = 'The existing Orientation record could not be linked to document verification. Please try again.';
                  header("Location: " . beneficiary_return_location('peso_staff_beneficiaries.php', $program_name_post));
                  exit();
...
<body class="staff-mobile-page peso-staff-beneficiaries-page">
  <div class="page-wrap">
      ...
      <section class="panel-card animate-fade-in" style="flex: 1; animation-delay: 0.1s; margin-top: 8px; display: flex; flex-direction: column;">
        <div class="records-toolbar">
            <div class="custom-tabs">
                <a href="peso_staff_beneficiaries.php?<?php echo h(build_query(['approval' => 'All', 'availment' => 'All', 'spes_group' => 'all', 'page' => 1])); ?>" class="tab-item <?php echo $approvalFilter === 'All' && $spesGroup === 'all' ? 'active' : ''; ?>">All Records</a>
                <a href="peso_staff_beneficiaries.php?<?php echo h(build_query(['approval' => 'Pending', 'availment' => 'All', 'spes_group' => 'all', 'page' => 1])); ?>" class="tab-item <?php echo $approvalFilter === 'Pending' && $spesGroup === 'all' ? 'active' : ''; ?>">Needs Approval</a>
                <a href="peso_staff_beneficiaries.php?<?php echo h(build_query(['approval' => 'Approved', 'availment' => 'All', 'spes_group' => 'all', 'page' => 1])); ?>" class="tab-item <?php echo $approvalFilter === 'Approved' && $spesGroup === 'all' ? 'active' : ''; ?>">Approved</a>
                <a href="peso_staff_beneficiaries.php?<?php echo h(build_query(['approval' => 'Rejected', 'availment' => 'All', 'spes_group' => 'all', 'page' => 1])); ?>" class="tab-item <?php echo $approvalFilter === 'Rejected' && $spesGroup === 'all' ? 'active' : ''; ?>">Rejected</a>
                <?php if (stripos($selectedProgramName, 'SPES') !== false): ?>
                <a href="peso_staff_beneficiaries.php?<?php echo h(build_query(['approval' => 'All', 'availment' => 'All', 'spes_group' => 'baby', 'page' => 1])); ?>" class="tab-item <?php echo $spesGroup === 'baby' ? 'active' : ''; ?>">SPES Baby</a>
                <a href="peso_staff_beneficiaries.php?<?php echo h(build_query(['approval' => 'All', 'availment' => 'All', 'spes_group' => 'graduate', 'page' => 1])); ?>" class="tab-item <?php echo $spesGroup === 'graduate' ? 'active' : ''; ?>">Graduated</a>
                <?php endif; ?>
            </div>
                      ...
                      <tr class="table-row-animate clickable-row" data-profile='<?php echo $profileData; ?>' onclick="window.openProfileModal(this)">
                          ...
                          <td style="text-align: center;" onclick="event.stopPropagation();">
                               <div class="action-menu-wrap" style="justify-content: center;">
                                  ...
                                  <?php endif; ?>
                                  <button type="button" class="btn-edit beneficiary-edit-record" onclick="openEditModal(this.closest('tr').getAttribute('data-profile'))" title="Edit beneficiary record">
                                      <i class="ph-bold ph-pencil-simple" aria-hidden="true"></i> Edit Record
                                  </button>
                              </div>
                          </td>
                      </tr>
...
<script src="spes_form_modal.js?v=20260926b"></script>
<script src="msme_form_modal.js?v=20260926b"></script>
<script>
(() => {
