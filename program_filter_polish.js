(function () {
  ...
  function initializeCategoryMenu(menu) {
    const form = menu.closest('form');
    const input = form ? form.querySelector('[data-category-input]') : null;
    const trigger = menu.querySelector('.program-category-trigger');
    const options = menu.querySelector('.program-category-options');
    const buttons = Array.from(menu.querySelectorAll('[data-category-value]'));
    if (!form || !input || !trigger || !options || !buttons.length) return;
    const close = (restoreFocus) => {
      options.hidden = true;
      trigger.setAttribute('aria-expanded', 'false');
    ...
    buttons.forEach((button) => button.addEventListener('click', () => {
      input.value = button.dataset.categoryValue || 'All';
      close(false);
      form.submit();
    }));
    options.addEventListener('keydown', (event) => {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('[data-category-menu]').forEach(initializeCategoryMenu);
  });
})();
