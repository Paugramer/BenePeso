...
function ensure_availment_status_schema(mysqli $conn): void {
  ...
  if (strpos($type, "enum(") !== 0 || (strpos($type, "'requirements received'") !== false && strpos($type, "'orientation'") !== false && strpos($type, "'salary distribution'") !== false && strpos($type, "'not qualified'") !== false)) return;
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if ($action === "approve_beneficiary") {
      ...
      if ($beneficiary_id > 0) {
          ...
          $fetch_query = "SELECT b.email, b.first_name, b.program_id, b.approval_status, p.program_name, p.slots 
          ...
          if ($row = $result->fetch_assoc()) {
              ...
              if ($stmt->execute()) {
                  ...
                  if (!empty($user_email) && strpos(strtolower($user_email), 'no email') === false) {
                      ...
                      $approve_headline = "Congratulations, " . h($first_name) . "!";
                       $approve_body = "
                           <p>We are pleased to inform you that your application for the <strong>{$program_name}</strong> has been successfully <strong>APPROVED</strong>.</p>
                           <p>You were selected after eligibility verification and review in application submission order, subject to the available program slots.</p>
                           <p>Please wait for further announcements or text messages from the PESO Vinzons office regarding the next steps and requirements you need to prepare.</p>
                       ";
                  ...
                  if ($total_slots > 0 && $approved_count >= $total_slots) {
                      ...
                      while ($pending_row = $pendingRes->fetch_assoc()) {
                          ...
                          if (!empty($pending_email) && strpos(strtolower($pending_email), 'no email') === false) {
                              ...
                              $reject_headline = "Program Slots Filled";
                              $reject_body = "
                                  <p>Dear " . h($pending_fname) . ",</p>
                                  <p>Thank you for your interest in the <strong>{$program_name}</strong>.</p>
                                  <p>We regret to inform you that the available slots have been filled by qualified applicants ahead in the application queue. Your application was not selected for this batch.</p>
                                  <p>We highly encourage you to apply for other available programs in the BENEPESO system. Thank you for understanding.</p>
                              ";
          ...
          if ($stmt->execute()) {
              ...
              if (!empty($user_email) && strpos(strtolower($user_email), 'no email') === false) {
                  ...
                  $reject_body = "
                      ...
                      <p>Thank you for applying to the <strong>{$program_name}</strong>.</p>
                      ...
                      <p style='margin-top: 20px;'>We encourage you to update your profile and apply for other suitable programs in the future.</p>
                  ";
      ...
      if ($beneficiary_id > 0) {
          ...
          if ($stmt) {
              ...
              if ($stmt->execute()) {
                  ...
                  $email_attempted = true;
                  $email_error = null;
                  $email_sent = sendBENEPESOStatusEmail($conn, $beneficiary_id, $availment_status, $email_error, $status_message, (string)$date_availed, $schedule_place);

                  $_SESSION["show_success_modal"] = true;
                  if (!$email_sent) {
                      $_SESSION["success_modal_message"] = "Availment status updated successfully, but the email notification could not be sent: " . ($email_error ?: "Unknown mail error.");
                  } else {
                      $_SESSION["success_modal_message"] = "Availment status updated successfully and email sent.";
                  }
                  $_SESSION["modal_icon"] = "✓";
              } else {
                  $_SESSION["show_error_modal"] = true;
              ...
              if ($stmt->execute()) {
                  ...
                  if ($action === "admin_update_beneficiary" && $previous_availment_status !== null && $previous_availment_status !== $availment_status) {
                      ...
                      if (sendBENEPESOStatusEmail($conn, $bid, $availment_status, $email_error)) {
                          ...
                          $_SESSION["success_modal_message"] .= " The status changed, but the email notification could not be sent: " . ($email_error ?: "Unknown mail error.");
                      }
                  }
                  $_SESSION["modal_icon"] = "✓";
              } else {
...
<head>
  ...
  <link rel="stylesheet" href="admin_beneficiaries.css?v=20260820-dependent-modal-polish">
...
<body>
  ...
  <aside class="side-area" id="sideArea">
    ...
    <nav class="nav-area">
      ...
      <a href="logout.php?role=admin" class="nav-item logout-item"><i class="ph-bold ph-sign-out"></i> Logout</a>
    </nav>
  </aside>
      ...
      <section class="panel-card animate-fade-in" style="flex: 1; animation-delay: 0.1s; margin-top: 8px; display: flex; flex-direction: column;">
        <div class="records-toolbar">
            <div class="custom-tabs">
                <a href="admin_beneficiaries.php?<?php echo h(build_query(['approval' => 'All', 'page' => 1])); ?>" class="tab-item <?php echo $approvalFilter === 'All' ? 'active' : ''; ?>">All Records</a>
                <a href="admin_beneficiaries.php?<?php echo h(build_query(['approval' => 'Pending', 'page' => 1])); ?>" class="tab-item <?php echo $approvalFilter === 'Pending' ? 'active' : ''; ?>">Needs Approval</a>
                <a href="admin_beneficiaries.php?<?php echo h(build_query(['approval' => 'Approved', 'page' => 1])); ?>" class="tab-item <?php echo $approvalFilter === 'Approved' ? 'active' : ''; ?>">Approved</a>
                <a href="admin_beneficiaries.php?<?php echo h(build_query(['approval' => 'Rejected', 'page' => 1])); ?>" class="tab-item <?php echo $approvalFilter === 'Rejected' ? 'active' : ''; ?>">Rejected</a>
            </div>
                              ...
                              <div class="action-menu-wrap" style="justify-content: center;">
                                  <?php if ($beneficiaryApproval === 'Pending'): ?>
                                      <form method="POST" style="margin: 0;" novalidate>
                                          ...
                                          <input type="hidden" name="program_name" value="<?php echo h($selectedProgramName); ?>">
                                          <button type="submit" class="btn-icon-approve" title="Approve"><i class="ph-bold ph-check"></i></button>
                                      </form>
                                      <button type="button" class="btn-icon-reject open-reject-modal" onclick="openRejectModal(<?php echo $bId; ?>)" title="Reject"><i class="ph-bold ph-x"></i></button>
...
<div class="modal" id="quickStatusModal" aria-hidden="true">
  ...
  <div class="modal-dialog modal-dialog-sm">
    <div class="modal-head-sm">
      <div>
        <div class="modal-title">Update Availment</div>
